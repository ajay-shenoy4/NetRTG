...
# Calculate distances and filter points 20 units or more from the mean
...
# Filter points 20 units or more from the mean
highlighted_points = data[data["Distance"] >= 20]
...
output_path = os.path.join(folder_path, "highlighted_lineups_20_units_sorted_Greater_50_Mins.csv")
...
# Highlight points 20 units away
plt.scatter(
    highlighted_points["OFFRTG"], 
    highlighted_points["DEFRTG"], 
    color="orange", 
    label="Points ≥ 20 Units from Mean With > 50 Mins Played", 
    edgecolor="black"
)
...
# Save the plot
plot_path = os.path.join(graph_path, "scatterplot_with_mean_and_highlights_20_units_annotated_Greater_50_Mins.png")
plt.savefig(plot_path, dpi=300)
plt.show()

# Print confirmation
print(f"Highlighted lineups (≥ 20 units from mean) have been saved to '{graph_path}'.")
